        ...
        <div class="bg-gray-700 py-8 px-8 rounded w-2/3 xl:w-1/2">
            ...
            <template v-else>
                <form @submit.prevent="submit">
                    ...
                    <span class="flex justify-center text-red-400 italic mt-4" v-if="errors.email">{{ errors.this.email[0] }}</span>

                    <div class="flex flex-col mt-4">
                        <label class="text-gray-50">New Password</label>
                        <input type="password" v-model="form.password" class="px-2 py-1 rounded" placeholder="New Password">
                    </div>
                    <span class="flex justify-center text-red-400 italic mt-4" v-if="errors.password">{{ errors.password[0] }}</span>

                    <div class="flex flex-col mt-4">
                        <label class="text-gray-50">Confirm New Password</label>
                        <input type="password" v-model="form.password_confirmation" class="px-2 py-1 rounded" placeholder="Confirm New Password">
                    </div>
